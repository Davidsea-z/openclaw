...
function findProjectRoot() {
  if (process.env.STAR_PROJECT_ROOT) {
    ...
    if (fs.existsSync(path.join(custom, "backend", "app.py"))) {
      return custom;
    }
  }
  ...
  let cursor = fromDir;
  for (let i = 0; i < 8; i += 1) {
    if (fs.existsSync(path.join(cursor, "backend", "app.py"))) {
      return cursor;
    }
  ...
  for (const c of candidates) {
    if (fs.existsSync(path.join(c, "backend", "app.py"))) {
      return c;
    }
  }
...
function spawnBackend(projectRoot) {
  ...
  const candidates = [];
  if (process.env.STAR_BACKEND_PYTHON) {
    candidates.push(process.env.STAR_BACKEND_PYTHON);
  }
...
async function enterMiniMode(projectRoot) {
  ...
  if (snapshot) {
    emitMini("mini-sync-state", { ...snapshot, ui_lang: currentUiLang });
  }
...
async function bootstrap() {
  ...
  if (!(await tcpReachable(BACKEND_HOST, BACKEND_PORT, 400))) {
    ...
    if (!ready) {
      console.warn("backend not ready within 20s");
    }
  } else {
