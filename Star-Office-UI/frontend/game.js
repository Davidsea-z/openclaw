...
function preload() {
  ...
  this.load.image(
    "office_bg",
    "/static/office_bg_small" + (supportsWebP ? ".webp" : ".png") + "?v={{VERSION_TIMESTAMP}}",
  );
  this.load.spritesheet(
    "star_idle",
    "/static/star-idle-spritesheet" + getExt("star-idle-spritesheet.png"),
    { frameWidth: 128, frameHeight: 128 },
  );
  this.load.spritesheet(
    "star_researching",
    "/static/star-researching-spritesheet" + getExt("star-researching-spritesheet.png"),
  ...
  this.load.spritesheet(
    "sofa_busy",
    "/static/sofa-busy-spritesheet" + getExt("sofa-busy-spritesheet.png"),
    { frameWidth: 256, frameHeight: 256 },
  );

  this.load.spritesheet("plants", "/static/plants-spritesheet" + getExt("plants-spritesheet.png"), {
    frameWidth: 160,
    frameHeight: 160,
  });
  this.load.spritesheet(
    "posters",
    "/static/posters-spritesheet" + getExt("posters-spritesheet.png"),
    { frameWidth: 160, frameHeight: 160 },
  );
  this.load.spritesheet(
    "coffee_machine",
    "/static/coffee-machine-spritesheet" + getExt("coffee-machine-spritesheet.png"),
    { frameWidth: 230, frameHeight: 230 },
  );
  this.load.spritesheet(
    "serverroom",
    "/static/serverroom-spritesheet" + getExt("serverroom-spritesheet.png"),
    { frameWidth: 180, frameHeight: 251 },
  );

  this.load.spritesheet(
    "error_bug",
    "/static/error-bug-spritesheet-grid" + (supportsWebP ? ".webp" : ".png"),
    { frameWidth: 180, frameHeight: 180 },
  );
  this.load.spritesheet("cats", "/static/cats-spritesheet" + (supportsWebP ? ".webp" : ".png"), {
    frameWidth: 160,
    frameHeight: 160,
  });
  ...
  this.load.spritesheet(
    "star_working",
    "/static/star-working-spritesheet-grid" + (supportsWebP ? ".webp" : ".png"),
    { frameWidth: 230, frameHeight: 144 },
  );
  this.load.spritesheet(
    "sync_anim",
...
function create() {
  ...
  // === 沙发（来自 LAYOUT）===
  sofa = this.add
    .sprite(LAYOUT.furniture.sofa.x, LAYOUT.furniture.sofa.y, "sofa_busy")
  ...
  const plaqueBg = game.add.rectangle(
    plaqueX,
    plaqueY,
    LAYOUT.plaque.width,
  ...
  const plaqueText = game.add
    .text(plaqueX, plaqueY, "War room", {
    ...
    .setOrigin(0.5);
  game.add
    .text(plaqueX - 190, plaqueY, "⭐", { fontFamily: "ArkPixel, monospace", fontSize: "20px" })
    .setOrigin(0.5);
  game.add
    .text(plaqueX + 190, plaqueY, "⭐", { fontFamily: "ArkPixel, monospace", fontSize: "20px" })
    .setOrigin(0.5);
  ...
  const poster = game.add
    .sprite(LAYOUT.furniture.poster.x, LAYOUT.furniture.poster.y, "posters", randomPosterFrame)
    .setOrigin(0.5);
  ...
  const cat = game.add
    .sprite(LAYOUT.furniture.cat.x, LAYOUT.furniture.cat.y, "cats", randomCatFrame)
  ...
  const coffeeMachine = this.add
    .sprite(LAYOUT.furniture.coffeeMachine.x, LAYOUT.furniture.coffeeMachine.y, "coffee_machine")
  ...
  serverroom = this.add
    .sprite(LAYOUT.furniture.serverroom.x, LAYOUT.furniture.serverroom.y, "serverroom", 0)
  ...
  const desk = this.add
    .image(LAYOUT.furniture.desk.x, LAYOUT.furniture.desk.y, "desk_v2")
  ...
  const flower = this.add
    .sprite(LAYOUT.furniture.flower.x, LAYOUT.furniture.flower.y, "flowers", randomFlowerFrame)
  ...
  // === 错误 bug（来自 LAYOUT）===
  const errorBug = this.add
    .sprite(LAYOUT.furniture.errorBug.x, LAYOUT.furniture.errorBug.y, "error_bug", 0)
  ...
  const starWorking = this.add
    .sprite(LAYOUT.furniture.starWorking.x, LAYOUT.furniture.starWorking.y, "star_working", 0)
  ...
  syncAnimSprite = this.add
    .sprite(LAYOUT.furniture.syncAnim.x, LAYOUT.furniture.syncAnim.y, "sync_anim", 0)
...
function update(time) {
  ...
  if (time - lastAgentsFetch > AGENTS_FETCH_INTERVAL) {
    fetchAgents();
  ...
  if (window.errorBug) {
    if (effectiveStateForServer === "error") {
      ...
      if (
        !window.errorBug.anims.isPlaying ||
        window.errorBug.anims.currentAnim?.key !== "error_bug"
      ) {
  ...
  if (syncAnimSprite) {
    if (effectiveStateForServer === "syncing") {
      if (
        !syncAnimSprite.anims.isPlaying ||
        syncAnimSprite.anims.currentAnim?.key !== "sync_anim"
      ) {
        syncAnimSprite.anims.play("sync_anim", true);
      }
    } else {
      if (syncAnimSprite.anims.isPlaying) {
        syncAnimSprite.anims.stop();
      }
      syncAnimSprite.setFrame(0);
    }
...
function showCatBubble() {
  ...
  if (window.catBubble) {
    window.catBubble.destroy();
    window.catBubble = null;
  }
...
function renderAgent(agent) {
  ...
  if (!agents[agentId]) {
    ...
    if (nameTag && nameTag.name === "nameTag") {
      ...
      nameTag.setFill(
        "#" +
          (NAME_TAG_COLORS[authStatus] || NAME_TAG_COLORS.default).toString(16).padStart(6, "0"),
      );
    }
    // 更新状态点颜色
