...
### 8) 昨日小记与 OpenClaw 日志同步

- 昨日小记：后端从 `memory/YYYY-MM-DD.md`（Star-Office-UI 上一级目录下的 `memory/`）读取并脱敏展示。
- 若希望「昨日小记」显示 OpenClaw 的对话日志，可定期运行同步脚本，把 OpenClaw 的 session transcript 写入上述目录：
  - `python3 scripts/sync-openclaw-logs-to-memo.py`：默认同步「昨天」的日志到 `memory/昨天日期.md`。
  - 可选 `--date YYYY-MM-DD`、`--dry-run`；环境变量 `OPENCLAW_HOME`、`MEMORY_DIR` 可覆盖路径。
...
- `GET /yesterday-memo`：昨日小记（来自 `memory/*.md`）
